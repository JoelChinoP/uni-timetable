...
<button
	class={`planner-event-enter absolute flex min-h-0 flex-col overflow-hidden rounded-[10px] border border-l-[3px] px-1.5 py-1 text-left shadow-card transition duration-150 hover:z-10 hover:border-accent ${event.conflictIds.length > 0 ? 'ring-2 ring-warning/35' : ''}`}
	type="button"
...
>
	<div class="flex min-w-0 items-center gap-1">
		<span class="truncate text-[10px] font-extrabold tracking-[0.08em] text-secondary uppercase"
			>{event.code}</span
		>
		{#if event.conflictIds.length > 0}<svg
				class="ml-auto h-3 w-3 shrink-0 fill-warning stroke-warning"
				viewBox="0 0 24 24"
				stroke-width="1.5"
				aria-hidden="true"><path d="M12 3 2.8 20h18.4L12 3Z" /></svg
			>{/if}
	</div>
